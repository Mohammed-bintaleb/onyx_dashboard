import 'package:flutter/material.dart';

class AppColors {
  // Text Colors
  static const Color lightText = Colors.black;
  static const Color lightLabel = Colors.grey;
  static const Color darkText = Colors.white;
  static const Color darkLabel = Color(0xFFB0BEC5);

  //Border Colors
  static const Color darkBorder = Color(0xFF444654);
  static const Color lightBorder = Color(0xFFE0E0E0);

  // card Colors
  static const Color lightCard = Colors.white;
  static const Color darkCard = Color(0xFF0A0E21);
  static const Color cardBackgroundLight = Color(0xFFF5F8FA);
  static const Color cardBackgroundDark = Color(0xFF0A0E21);

  // Background
  static const Color darkFieldBg = Color(0xFF2C2F4A);
  static final Color lightFieldBg = Colors.grey[200]!;
  static const Color lightBackground = Colors.white;
  static const Color darkBackground = Color(0xFF0A0E21);

  // Common Colors
  static const Color iconGrey = Color(0xFF757575);

  //hover Colors
  static const lightHover = Color(0xFFF5F5F5);
  static const darkHover = Color(0xFF2C2F4A);
}
